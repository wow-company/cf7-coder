...
class wpcf7_Coder_Extension_Activation {
    ...
    public function __construct( $plugin_path, $plugin_file ) {
        ...
        if( isset( $plugins[$this->plugin_path . '/' . $this->plugin_file]['Name'] ) ) {
            ...
            $this->plugin_name = __( 'This plugin', 'cf7-coder' );
        }
    ...
    public function missing_wpcf7_notice() {
        if( $this->has_wpcf7 ) {
            ...
            $link = '<a href="' . $url . '">' . __( 'activate it', 'cf7-coder' ) . '</a>';
        } else {
            $url  = esc_url( wp_nonce_url( self_admin_url( 'update.php?action=install-plugin&plugin=wpcf7' ), 'install-plugin_wpcf7' ) );
            $link = '<a href="' . $url . '">' . __( 'install it', 'cf7-coder' ) . '</a>';
        }
        
        echo '<div class="error"><p>' . $this->plugin_name . sprintf( __( ' requires Contact Form 7! Please %s first and then activate this.', 'cf7-coder' ), $link ) . '</p></div>';
    }
}
